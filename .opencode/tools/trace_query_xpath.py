...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Export xctrace XML for a specific XPath from a .trace file"
    )
    parser.add_argument(
        "--trace-path",
    ...
    parser.add_argument(
        "--xpath",
        required=True,
        help="XPath passed through to `xctrace export --xpath`",
    )
    return parser
...
def export_trace_xpath(trace_path: Path, xpath: str) -> dict:
...
def main() -> int:
    ...
    with _buffer_stderr_on_success():
        ...
        payload = export_trace_xpath(trace_path, args.xpath)
